...
aspect_model = nn.Copy(float,float)
par_model = nn.ParallelTable()
par_model:add(model)
par_model:add(aspect_model)
new_model = nn.Sequential()
new_model:add(par_model)
new_model:add(nn.JoinTable(2,2))
for i = 1, args.mlp_hidden_layers do
    new_model:add(nn.Dropout())
    new_model:add(nn.Linear((args.dim + nAspects), args.mlp_hidden_units))
    new_model:add(nn.ReLU())
end
new_model:add(nn.Dropout())
new_model:add(nn.Linear(args.mlp_hidden_units, nPolarities))
new_model:add(nn.SoftMax())
