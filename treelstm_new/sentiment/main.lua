...
Training script for sentiment classification on the SST dataset.
  ...
  -i,--idx (number)                     Cross validation index
]]
...
if args.dataset == 'restaurant' then
  data_dir = string.format('all_data/restaurant/rest_%i/', args.idx)
  aspects = 12
  aspect_map = treelstm.read_aspect_map('all_data/restaurant/rest.category')
elseif args.dataset == 'laptop' then
  data_dir = string.format('all_data/laptop/lapt_%i/', args.idx)
  aspects = 81
  aspect_map = treelstm.read_aspect_map('all_data/laptop/lapt.category')
end
...
local train_dataset = treelstm.read_sentiment_dataset(train_dir, vocab, fine_grained, dependency, aspect_map, aspects)
local dev_dataset = treelstm.read_sentiment_dataset(dev_dir, vocab, fine_grained, dependency, aspect_map, aspects)
...
printf('max epochs = %d\n', num_epochs)
model:print_config()
--local dev_predictions = model:get_probs(dev_dataset)
...
for i = 1, num_epochs do
  ...
  --local dev_prob = model:get_probs(dev_dataset)
  --print(dev_prob)
...
while true do
  ...
  model_save_path = string.format(
    treelstm.models_dir .. '/' .. args.dataset .. '/sent-%s.%s.%dl.%dd.%d.th', args.model, subtask, args.layers, args.dim, file_idx)
