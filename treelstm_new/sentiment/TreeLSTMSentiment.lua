  ...
  for i = 1, dataset.size, self.batch_size do
    ...
    local feval = function(x)
      ...
      for j = 1, batch_size do
        ...
        local aspects = dataset.aspects[idx]
        --print(aspects)
        --local aspects = dataset.aspects[{{idx},{}}]

        local inputs = self.emb:forward(sent)
        local _, tree_loss = self.treelstm:forward(tree, inputs, aspects, true)
        loss = loss + tree_loss
        local input_grad = self.treelstm:backward(tree, inputs, aspects, {zeros, zeros})
...
function TreeLSTMSentiment:predict(tree, sent, aspects)
  self.treelstm:evaluate()
  local prediction
  local inputs = self.emb:forward(sent)
  self.treelstm:forward(tree, inputs, aspects, true)
  local output = tree.final_output
...
function TreeLSTMSentiment:prob(tree, sent, aspects)
  self.treelstm:evaluate()
  local inputs = self.emb:forward(sent)
  self.treelstm:forward(tree, inputs, aspects, true)
  local output = tree.final_output
...
function TreeLSTMSentiment:predict_dataset(dataset)
  ...
  for i = 1, dataset.size do
    xlua.progress(i, dataset.size)
    predictions[i] = self:predict(dataset.trees[i], dataset.sents[i], dataset.aspects[i])
  end
  return predictions
end

function TreeLSTMSentiment:get_probs(dataset)
  local probs = torch.Tensor(dataset.size, 3)
  for i = 1, dataset.size do
    xlua.progress(i, dataset.size)
    probs[i] = self:prob(dataset.trees[i], dataset.sents[i], dataset.aspects[i])
  end
  return probs
end
