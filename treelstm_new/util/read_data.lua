...
function treelstm.read_aspect_map(path)
    local map_file = io.open(path, 'r')
    map = {}
    local count = 0
    local len = 0
    while true do
        local line = map_file:read()
        if line == nil then break end
        len = len + 1
...
function treelstm.read_aspects(path, map, count)
    local aspect_file = io.open(path, 'r')
    local len = 0
    while true do
        local line = aspect_file:read()
        if line == nil then break end
        len = len + 1
    end
    aspect_file:seek('set')
    local aspects = torch.zeros(len, count)
    len = 0
    while true do
        local line = aspect_file:read()
        if line == nil then break end
        ...
        if map[line] ~= nil then
            ...
            aspect = map['LAPTOP#GENERAL']
        end
        aspects[len][aspect] = 1
    end
    aspect_file:close()
    return aspects
end
...
function treelstm.read_sentiment_dataset(dir, vocab, fine_grained, dependency, map, aspect_num)
  local dataset = {}
  dataset.vocab = vocab
  ...
  if dependency then
    ...
    trees = treelstm.read_trees(dir .. 'cparents.txt', dir .. 'clabels.txt')
    for _, tree in ipairs(trees) do
      set_spans(tree)
    end
  end

  local aspects = treelstm.read_aspects(dir .. 'aspects.txt', map, aspect_num)
