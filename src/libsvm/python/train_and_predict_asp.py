...
with open('misc_data/{}_train.asp.dat.{}'.format(sys.argv[1],sys.argv[3])) as train_data:
    ...
    train_labels=[]
    with open('misc_data/{}_train.asp.label.{}'.format(sys.argv[1],sys.argv[3])) as label_file:
    ...
    svm_save_model('src/libsvm/SVMmodel/{}.model.{}'.format(sys.argv[1],sys.argv[3]),svm_classify_model)


#Predicting stage
with open('misc_data/{}_te.asp.dat.{}'.format(sys.argv[1],sys.argv[3])) as test_data:
    ...
    bow_corpus_numeric = bow_corpus_numeric.tolist()
    svm_classify_model= svm_load_model('src/libsvm/SVMmodel/{}.model.{}'.format(sys.argv[1],sys.argv[3]))
...
with open('output/{}_asp.out.{}'.format(sys.argv[1],sys.argv[3]),'w') as ans:
    # Among this label , a label is only occur at once (MULTIMEDIA_DEVICE#MISC) , drop it!
    for p_dis in p_val:
        ans_list=[]
