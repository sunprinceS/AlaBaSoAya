...
python scripts/preprocess/preprocess_asp.py $dom train 
#produce SVMmodel/lap.model
python src/libsvm/python/train_asp.py $dom
...
python scripts/preprocess/preprocess_asp.py $dom te
python src/libsvm/python/predict_asp.py $dom $thr

echo -e "***** Produce result (xml format) in slot1  *****"
python scripts/submit/submit_asp.py $dom
# Stage 2 Training
...
java -cp ./A.jar absa16.Do Eval -prd "output/${dom}_asp.xml" -gld "tmp_data/teGld.xml" -evs 1 -phs A -sbt SB1
